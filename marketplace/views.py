# Imports related to Django and third-party libraries
...
from .forms import CommentForm, EmailPostForm, SearchForm, UserRegistrationForm, UserEditForm, ProfileEditForm, \
    LoginForm, RatingForm, SellerReviewForm
...
from .mixins import CachePageMixin, StaffRequiredMixin, FormValidMixin, SuccessMessageMixin
...
def product_list1(request, tag_slug=None):
    ...
    return render(request, 'marketplace/product/list.html', {'products': products, 'tag': tag})
...
def product_detail(request, year, month, day, slug):
    ...
    return render(request, 'marketplace/product/detail.html', {
        'product': product,
        'comments': comments,
        'form': form,
...
def product_comment(request, product_id):
    ...
    if form.is_valid():
        ...
        comment.save()
    return render(request, 'marketplace/product/comment.html', {'product': product, 'form': form, 'comment': comment})
...
def product_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            results = Product.published.filter(Q(title__icontains=query) | Q(description__icontains=query)).order_by(
                '-updated')
    return render(request, 'marketplace/product/search.html', {'form': form, 'query': query, 'results': results})
...
def product_share(request, product_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            subject = f"{cd['name']} ({cd['email']}) recommends you read {product.title}"
            message = f"Read {product.title} at {product_url}\n\n{cd['name']}\'s comments: {cd['comments']}"
            send_mail(subject, message, 'nedilja90@gmail.com', [cd['to']])
            sent = True
    else:
        form = EmailPostForm()

    return render(request, 'marketplace/product/share.html', {'product': product, 'form': form, 'sent': sent})
...
# Class-based views
class ProductListView(CachePageMixin, ListView):
    model = Product
...
class SellerListView(CachePageMixin, ListView):
    model = SellerProfile
    template_name = 'marketplace/sellers/seller_list.html'
    context_object_name = 'sellers'


class SellerDetailView(StaffRequiredMixin, DetailView):
    model = SellerProfile
